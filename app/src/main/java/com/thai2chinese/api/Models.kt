...
fun WhisperResponse.toSentences(offset: Double = 0.0): List<Sentence> {
    return segments.filter { it.no_speech_prob < 0.3 }.map { seg ->
        val words = if (seg.words.isNotEmpty()) {
            ...
            tokens.mapIndexed { i, t -> Word(text = t, start = offset + seg.start + i * wordDur, end = offset + seg.start + (i + 1) * wordDur) }
        }
