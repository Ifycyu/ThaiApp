    ...
    private suspend fun processVideo(videoUri: String, filename: String) {
        try {
            ...
            val enrichedSentences = sentences.mapIndexed { idx, sentence ->
                ...
                if (enriched.translation.isBlank()) {
                    try { withContext(Dispatchers.IO) { ThaiWordApi.translate(sentence.text, twUrl, headers) }.let { enriched.copy(translation = it.translated) } }
                    catch (_: Exception) { enriched }
                } else enriched
            }
